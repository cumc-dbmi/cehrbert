...
def main(
    ...
    popular_numeric_labs = popular_labs \
        .withColumn('is_numeric', F.col('numeric_percentage') >= 0.5) \
        .where('is_numeric') \
        .withColumn('rn', F.row_number().over(W.orderBy(F.desc('freq')))) \
        .where(F.col('rn') <= num_of_numeric_labs) \
        .drop('rn')

    popular_categorical_labs = popular_labs \
        .withColumn('is_categorical', F.col('categorical_percentage') >= 0.5) \
        .where('is_categorical') \
        .withColumn('is_numeric', ~F.col('is_categorical')) \
        .withColumn('rn', F.row_number().over(W.orderBy(F.desc('freq')))) \
        .where(F.col('rn') <= num_of_categorical_labs) \
        .drop('is_categorical').drop('rn')
